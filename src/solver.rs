use crate::board::{BoardCell, BoardCellState, BoardCells};
...
impl Solver {
    ...
    pub fn start(&mut self) {
        rayon::spawn({
            ...
            move || {
                log::info!("Start of Main Solver Thread");
                let (sender, receiver) = channel();
                let live_solvers = Arc::new(AtomicUsize::new(0));
                loop {
                    log::info!("Main Solver Thread: Loop beginning");
    ...
    pub fn update(&mut self, board: &BoardCells) {}
...
impl SolvableBoard {
    fn aux_get_verified(board: &BoardCells) -> HashSet<(usize, usize)> {
        board
            .iter()
            .enumerate()
    ...
    pub fn from_board(board: &BoardCells) -> Self {
        Self {
            board: board.clone(),
